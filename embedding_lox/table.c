...
static void adjust_capacity(LoxVM* vm, Table* table, int capacity) {
...
bool table_set(LoxVM* vm,Table* table, ObjString* key, Value value){
...
void table_add_all(LoxVM* vm,Table* from, Table* to) {
...
void mark_table(LoxVM* vm,Table* table){
    for(int i = 0; i < table->capacity; i++){
        Entry* entry = &table->entries[i];
